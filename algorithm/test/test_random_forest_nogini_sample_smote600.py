...
if __name__ == "__main__":
    ...
    smote_processor = Smote(new_train_df[new_train_df[target_key]==1],N=600,k=5)
    ...
    print('-----------------------------------------------------------------------------------------------------')
    param_test3 = {'n_estimators':range(20,600,20),'max_depth':range(2,16,2),'min_samples_split':range(20,200,20),       'max_features': ["auto","sqrt",None],'random_state':[1,49,99]}
    gsearch3 = GridSearchCV(estimator=RandomForestClassifier(min_samples_leaf=2, oob_score=True),
                            param_grid=param_test3, scoring='roc_auc', iid=False, cv=5)
    gsearch3.fit(X_train,y_train)
    for item in gsearch3.grid_scores_:
        print(item)
